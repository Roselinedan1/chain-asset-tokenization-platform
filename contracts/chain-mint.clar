...
;; Emergency function to transfer asset ownership (only contract owner)
    ...
    (if (not (is-contract-owner))
      ...
      (if (is-none asset-data)
        ...
        (begin
          ...
          ;; If tokenized, transfer all tokens to new owner
          (if (is-asset-tokenized asset-id)
            (let (
              (token-data (map-get? asset-tokens { asset-id: asset-id }))
              (current-owner (get owner (unwrap-panic asset-data)))
            )
              (if (is-some token-data)
                (begin
                  ;; Transfer all tokens to new owner
                  (update-balances asset-id current-owner new-owner (get total-supply (unwrap-panic token-data)))
                  (ok { asset-id: asset-id, new-owner: new-owner, tokens-transferred: true })
                )
                (ok { asset-id: asset-id, new-owner: new-owner, tokens-transferred: false })
              )
            )
            (ok { asset-id: asset-id, new-owner: new-owner, tokens-transferred: false })
          )
